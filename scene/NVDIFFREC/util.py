...
# Must be set before importing imageio which wraps opencv
...
def reflect2(x, n):
    NdotV = torch.sum(x*n, dim=-1, keepdim=True)
    w_k = 2*n*NdotV - x
    return w_k, NdotV
...
def save_image(fn, x : np.ndarray):
    try:
        if os.path.splitext(fn)[1] == ".png":
            ...
            cv2.imwrite(fn, cv2.cvtColor(np.clip(np.rint(x * 255.0), 0, 255).astype(np.uint8), cv2.COLOR_RGB2BGR)) # Low compression for faster saving
        else:
            # imageio.imwrite(fn, np.clip(np.rint(x * 255.0), 0, 255).astype(np.uint8))
            cv2.imwrite(fn, cv2.cvtColor(np.clip(np.rint(x * 255.0), 0, 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
    except Exception as err:
        raise RuntimeWarning(f"WARNING: FAILED to save image {fn} with: {err}")
