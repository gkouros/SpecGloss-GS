...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], relight=False, no_cameras=False):
        ...
        if self.loaded_iter:
            print("Loading checkpoints from iteration", self.loaded_iter)
            ...
            mesh_path = os.path.join(self.model_path, "mesh")
            if os.path.exists(mesh_path) and len(os.listdir(mesh_path)) > 0:
                mesh_iter = searchForMaxIteration(mesh_path)
                self.gaussians.load_mesh_from_ply(mesh_path, mesh_iter)
        else:
            self.gaussians.create_from_pcd(scene_info.point_cloud, self.cameras_extent)
            
        # update prefiltered envmap
    ...
    def getTrainCameras(self, scale=1.0):
        return self.train_cameras[scale] if self.train_cameras else []

    def getTestCameras(self, scale=1.0):
        return self.test_cameras[scale] if self.test_cameras else []