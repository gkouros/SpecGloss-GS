	...
	const bool debug)
{
  ...
  const int C = (M == 0) ? NUM_CHANNELS : M;
	...
	const bool debug) 
{
  ...
  if (M < 1 || M > NUM_CHANNELS) {
	throw std::runtime_error("Channel count out of range");
  }
  ...
  torch::Tensor dL_dsh = torch::zeros({P, C}, means3D.options());
